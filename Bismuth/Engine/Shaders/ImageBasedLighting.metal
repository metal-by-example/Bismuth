...
// This is upside-down, but it does what we want in context, so leave it alone for now.
static float2 EquirectUVFromCubeDirection(float3 dir) {
    return float2 { atan2(dir.z, dir.x) / M_PI_F * 0.5f + 0.5f, 1.0f - acos(dir.y) / M_PI_F };
}
...
static float3x3 TangentSpaceFromNormal(float3 normal) {
    ...
    float NdotY = dot(normal, float3(0.0f, 1.0f, 0.0f));
    float epsilon = 0.0000001f;
    if (1.0 - abs(NdotY) <= epsilon) {
...
// Ref. https://dassaultsystemes-technology.github.io/EnterprisePBRShadingModel/spec-2021x.md.html#components/sheen
static float3 CombinedF0OffsetAndBias(uint width, uint sampleCount, float NdotV, float roughness)
{
    ...
    return 4.0f * float3(A, B, C * 2.0f * M_PI_F) / float(sampleCount);
}
...
// GGX scale and bias are stored in r and g channels, respectively, while Charlie sheen scale is
// stored in the b channel.
